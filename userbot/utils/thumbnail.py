...
async def gen_thumb(thumbnail, title, userid, ctitle):
        ...
        async with session.get(thumbnail) as resp:
            if resp.status == 200:
                f = await aiofiles.open(
                    f"userbot/utils/resource/thumb{userid}.png", mode="wb"
                )
    ...
    Image.alpha_composite(image5, image6).save(f"userbot/utils/resource/temp{userid}.png")
    img = Image.open(f"userbot/utils/resource/temp{userid}.png")
    draw = ImageDraw.Draw(img)
    font = ImageFont.truetype("userbot/utils/styles/Roboto-Light.ttf", 52)
